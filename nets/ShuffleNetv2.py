        ...
        for stage_idx in range(len(self.stage_repeat_num)):
            if(stage_idx == 0):
                ...
                for i in range(repeat_num):
                    if i == 0:
                        self.stages2.append(ShuffleBlock(in_c, out_c, downsample=True))
                    else:
                        self.stages2.append(ShuffleBlock(in_c, in_c, downsample=False))
                    in_c = out_c
            elif (stage_idx == 1):
                out_c = self.out_channels[2 + stage_idx]
                repeat_num = self.stage_repeat_num[stage_idx]
                for i in range(repeat_num):
                    if i == 0:
                        self.stages3.append(ShuffleBlock(in_c, out_c, downsample=True))
                    else:
                        self.stages3.append(ShuffleBlock(in_c, in_c, downsample=False))
                    in_c = out_c
            else:
                out_c = self.out_channels[2 + stage_idx]
                repeat_num = self.stage_repeat_num[stage_idx]
                for i in range(repeat_num):
                    if i == 0:
                        self.stages4.append(ShuffleBlock(in_c, out_c, downsample=True))
                    else:
                        self.stages4.append(ShuffleBlock(in_c, in_c, downsample=False))
                    in_c = out_c
...
def shufflenetv2_0_5(pretrained=None):
...
def shufflenetv2_1_0(pretrained=None):
    """Constructs a ResNet-101 model.
...
def shufflenetv2_1_5(pretrained=None):
    """Constructs a ResNet-101 model.
...
def shufflenetv2_2_0(pretrained=None):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ShuffleNetv2(2)
    if pretrained:
        model.load_state_dict(torch.load(pretrained), strict=False)
    return model